function elasticText() {
  ...
  function mFn(obj) {
    ...
    if(!(effact in ef)) {
      throw new Error('没有此效果名称');
    }
    if(typeof du !== 'number') {
      throw new Error('持续时间类型错误')
    }
    ...
    tf.innerHTML = '';
    textBox.style = "width:" + fs * arr.length + "px;font-size:" + fs + "px;cursor:pointer;color:" + colr + ";position:relative;margin:0 auto;";
